...
// ../../node_modules/outvariant/lib/index.mjs
var POSITIONALS_EXP3 = /(%?)(%([sdjo]))/g;
function serializePositional3(positional, flag) {
  switch (flag) {
    case "s":
      return positional;
    case "d":
...
function format3(message2, ...positionals) {
  if (positionals.length === 0) {
    return message2;
  }
  let positionalIndex = 0;
  let formattedMessage = message2.replace(
    ...
    (match2, isEscaped, _, flag) => {
      const positional = positionals[positionalIndex];
      const value = serializePositional3(positional, flag);
      if (!isEscaped) {
        positionalIndex++;
        return value;
      }
...
var STACK_FRAMES_TO_IGNORE3 = 2;
function cleanErrorStack3(error3) {
  if (!error3.stack) {
    return;
  }
  const nextStack = error3.stack.split("\n");
  nextStack.splice(1, STACK_FRAMES_TO_IGNORE3);
  error3.stack = nextStack.join("\n");
}
var InvariantError3 = class extends Error {
  constructor(message2, ...positionals) {
    super(message2);
    this.message = message2;
    this.name = "Invariant Violation";
    this.message = format3(message2, ...positionals);
    cleanErrorStack3(this);
  }
};
var invariant3 = (predicate, message2, ...positionals) => {
  if (!predicate) {
    throw new InvariantError3(message2, ...positionals);
  }
};
invariant3.as = (ErrorConstructor, predicate, message2, ...positionals) => {
  if (!predicate) {
    const isConstructor = ErrorConstructor.prototype.name != null;
    const error3 = isConstructor ? new ErrorConstructor(format3(message2, positionals)) : ErrorConstructor(format3(message2, positionals));
    throw error3;
  }
};
