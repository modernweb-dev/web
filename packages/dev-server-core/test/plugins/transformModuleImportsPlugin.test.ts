...
describe('transformImports()', () => {
  ...
  it('resolves imports to a file with bare import', async () => {
    ...
    assert.deepEqual(result, "import 'RESOLVED__my-module/bar/index.js");
  });
  ...
  it('throws a syntax error on invalid imports', async () => {
    ...
    try {
      ...
      thrown = true;
      assert.equal((error as PluginSyntaxError).message, 'Syntax error');
      assert.equal((error as PluginSyntaxError).filePath, '/root/my-file.js');
      assert.equal((error as PluginSyntaxError).column, 16);
      assert.equal((error as PluginSyntaxError).line, 3);
    }
...
describe('transformImport', () => {
  ...
  it('transform comes after resolving imports', async () => {
    ...
    try {
      ...
      assert.deepEqual(receivedImports, ['RESOLVED__my-module', 'RESOLVED__./src/local-module.js']);
    } finally {
      server.stop();
    }
  });
