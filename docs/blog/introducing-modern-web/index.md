...
While we try to avoid complex builds during development, for production optimizations they are still a requirement. Here too things can get pretty complex. Through plugins and guides, we will try to make it easier to integrate with buildless development workflows.

A good example here is [@open-wc/rollup-plugin-html](https://www.npmjs.com/package/@open-wc/rollup-plugin-html) which we will move into the `@web` namespace. This plugin enables running rollup on an existing webpage, bundling, and optimizing any modules scripts inside. It works with single pages, but also with multiple HTML pages code splitting and shared the common code between them.
...
To add interactivity to our page we started using web components, and applied rollup with our [HTLM](https://www.npmjs.com/package/@open-wc/rollup-plugin-html) and [workbox](https://www.npmjs.com/search?q=rollup-plugin-workbox) plugins to the HTML pages generated by 11ty.
...
By combining existing tools in this way, we think we've made something very powerful that will be useful for other people as well. That's why we started a child project codenamed Rocket in the modern web family.
